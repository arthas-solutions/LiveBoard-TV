...
- Refresh client toutes les 60s (`useLiveData`).
- Route handlers Next.js avec cache/revalidate:
  - `/api/weather` -> `revalidate=60`
  - `/api/trains/departures` -> `revalidate=60`
  - `/api/trains/arrivals` -> `revalidate=60`
  - `/api/disruptions` -> `revalidate=60`
  - `/api/quote-of-the-day` -> `revalidate=3600`
